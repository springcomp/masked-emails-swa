...
export class CreateOrUpdateMaskedEmailAddressDialogComponentBase {
  ...
  protected minPasswordLength: number = 10;
    ...
    this.addressForm = this.formBuilder.group<CreateOrUpdateMaskedEmailAddressDialogData>({
      name: new FormControl(name === undefined ? '' : name, { validators: Validators.required, nonNullable: true }),
      description: new FormControl(description === undefined ? '' : description, { nonNullable: false }),
      password: new FormControl('', { validators: pristineOrminLength(this.minPasswordLength), nonNullable: false }),
    });
  }
